...
\`\`\``,
  ...
  async getCachePerformance(req: Request, res: Response): Promise<void> {
    try {
      const { cachePerformanceMonitor } = await import('../utils/cachePerformanceMonitor');
      ...
      res.json({
        ...
        performance: {
          ...
          topPerformers,
          memoryInfo: await redisCacheService.getCacheStats()
        }
      });
    } catch (error: any) {
      res.status(500).json({
